...
tic
opt = struct;
opt.data_dir = 'D:\DATA\malcolm_data\neuropix_data\MC9_20201211_optotagging2_g1'; % dataset to process
opt.dt = 0.02; % 50 Hz sampling for downsampled velocity
% opt.lick_thresh = 3; % voltage threshold for detecting licks (typical is 2.5)
opt.save_dir = 'D:\processed_neuropix_data'; % where to save processed data
opt.save_name = 'MC9_20201211_optotagging2.mat'; % file to save processed data to
...
ch_rews = 2; % reward valve opening
ch_laser = 3; % 'events' signalled from within DAQ for patch cue appearing, etc
...
save(fullfile(opt.save_dir,opt.save_name),...
    'sp','rew_ts','rew_dur_ms','laser_onset_ts','laser_offset_ts','laser_dur_ms');
fprintf('done.\n');
toc